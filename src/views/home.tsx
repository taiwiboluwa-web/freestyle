    ...
    <main className={`bluehaven-experience ${flavor === "blue" ? "blue-theme" : ""}`}>
      ...
      <header className="soda-header">
        ...
        <nav className={`soda-nav ${menuOpen ? "open" : ""}`}>
          <a href="#top" onClick={() => setMenuOpen(false)}>Home</a><a href="#work" onClick={() => setMenuOpen(false)}>Work</a><a href="#services" onClick={() => setMenuOpen(false)}>Services</a><a href="#about" onClick={() => setMenuOpen(false)}>About</a><a href="#contact" onClick={() => setMenuOpen(false)}>Contact</a>
        </nav>
      ...
      <section id="top" className="soda-hero">
        ...
        <div className="soda-hero-left">
          <p className="eyebrow">01 · CREATIVE STUDIO · LAGOS / NIGERIA</p>
          <h1><span>Beyond</span><br />limits.</h1>
          <p className="soda-description">BlueHaven Studios builds bold identities, digital experiences and media that make brands impossible to ignore.</p>
          <a className="soda-pill" href="#work">Explore our work <b>+</b></a>
        ...
        <div className="soda-hero-right">
          <div className="soda-cards">
            <button className={`soda-card ${flavor === "classic" ? "active" : ""}`} onClick={() => setFlavor("classic")}><img src={`${ASSET}/Green%20Soda.png`} alt="BlueHaven creative" /><span>Creative</span><small>Brand & Design</small></button>
            <button className={`soda-card ${flavor === "blue" ? "active" : ""}`} onClick={() => setFlavor("blue")}><img src={`${ASSET}/Blue%20Soda.png`} alt="BlueHaven digital" /><span>Digital</span><small>Web & Experiences</small></button>
          </div>
          <h2><span>Creativity</span><br />that moves.</h2>
        </div>
        <div className="hero-scroll">SCROLL TO DISCOVER <span>↓</span></div>
      </section>

      <section id="work" className="soda-section work-section"><div className="soda-section-top"><span>02 / Selected work</span><span>Built by BlueHaven</span></div><div className="soda-work-grid">{projects.map((project, i) => <article className={`soda-work-card card-${i + 1}`} key={project.title}><a href="#contact" className="soda-work-image" style={{ backgroundImage: `url(${project.image})` }}><span>0{i + 1}</span><b>↗</b></a><h3>{project.title}</h3><p>{project.type}</p></article>)}</div></section>

      <section id="services" className="soda-section dark-section"><div className="soda-section-top"><span>03 / Capabilities</span><span>What we make</span></div><h2 className="big-statement">We make <em>ideas</em><br />feel real.</h2><div className="soda-services">{["Brand strategy & identity", "Websites & digital experiences", "Graphic design & campaigns", "Photography & videography", "Livestreaming & broadcast", "Content & creative direction"].map((s, i) => <div key={s}><span>0{i + 1}</span><b>{s}</b><i>↗</i></div>)}</div></section>

      <section id="about" className="soda-section about-section"><div className="soda-section-top"><span>04 / The studio</span><span>Creating beyond limits</span></div><div className="about-grid"><h2>Different<br /><em>by design.</em></h2><div><p>BlueHaven Studios is a multidisciplinary creative studio connecting brand strategy, design, technology, media and storytelling.</p><p>From school campaigns to business identities, websites and live productions, we turn ambitious ideas into experiences people remember.</p><a href="#contact">Start a conversation ↗</a></div></div></section>

      <section id="contact" className="soda-contact-section"><div className="soda-section-top"><span>05 / Let&apos;s make something</span><span>Available for projects</span></div><h2>Have a brave<br /><em>idea?</em></h2><div className="contact-links"><a href="mailto:bluehavens.studios@gmail.com">bluehavens.studios@gmail.com <span>↗</span></a><a href="tel:+2348068483718">+234 806 848 3718 <span>↗</span></a><a href="https://bluehavens.name.ng" target="_blank" rel="noreferrer">bluehavens.name.ng <span>↗</span></a></div><footer><span>BLUEHAVEN STUDIOS®</span><span>CREATIVITY BEYOND LIMITS</span><span>LAGOS / NIGERIA</span></footer></section>
    </main>
  );
};

...
export const HomeView = () => {
  ...
  useEffect(() => { const script = document.createElement("script"); script.type = "module"; script.src = "https://unpkg.com/@google/model-viewer/dist/model-viewer.min.js"; document.head.appendChild(script); return () => script.remove(); }, []);
  ...
  return (
    <main className={`bluehaven-experience ${flavor === "blue" ? "blue-theme" : ""}`}>
      ...
      <section id="experience" className="loopstack-section"><div className="loopstack-video"><video autoPlay muted loop playsInline><source src={`${LOOPSTACK_ASSET}/flower.mp4`} type="video/mp4" /></video></div><img className="loopstack-gradient" src={`${LOOPSTACK_ASSET}/black_gradient.svg`} alt="" /><div className="loopstack-content"><p className="loopstack-eyebrow">02 · DIGITAL EXPERIENCE</p><h2>Creating beyond<br /><em>limits.</em></h2><p>Digital experiences with motion, atmosphere and personality. We blend technology, design and storytelling to turn a website into an experience.</p><a className="loopstack-cta" href="mailto:bluehavens.studios@gmail.com"><span>Start a project</span><i /></a></div><div className="loopstack-footer"><span>BLUEHAVEN STUDIOS</span><span>CREATIVITY BEYOND LIMITS</span><span>LAGOS / NIGERIA</span></div><div className="loopstack-wordmark">BlueHaven</div></section>
      <section id="cosmic" className="cosmic-section"><iframe className="cosmic-frame" src="/cosmic-dust.html" title="BlueHaven Cosmic Dust" loading="lazy" /><div className="cosmic-overlay"><div><p>03 · COSMIC DUST</p><h2>Ideas in <em>motion.</em></h2><span>Procedural light · Digital atmosphere · Endless drift</span></div><a href="mailto:bluehavens.studios@gmail.com">Create an experience ↗</a></div></section>
      <section id="laocoon" className="laocoon-section"><iframe className="laocoon-frame" src="/laocoon.html" title="Laocoön — Bronze and Time" loading="lazy" /><div className="laocoon-label"><span>04 · LAOCOÖN</span><b>Bronze and Time</b></div></section>
      <section id="work" className="soda-section work-section"><div className="soda-section-top"><span>05 / Selected work</span><span>Built by BlueHaven</span></div><div className="soda-work-grid">{projects.map((project, i) => <article className={`soda-work-card card-${i + 1}`} key={project.title}><a href="#contact" className="soda-work-image" style={{ backgroundImage: `url(${project.image})` }}><span>0{i + 1}</span><b>↗</b></a><h3>{project.title}</h3><p>{project.type}</p></article>)}</div></section>
      <section id="services" className="soda-section dark-section"><div className="soda-section-top"><span>06 / Capabilities</span><span>What we make</span></div><h2 className="big-statement">We make <em>ideas</em><br />feel real.</h2><div className="soda-services">{["Brand strategy & identity", "Websites & digital experiences", "Graphic design & campaigns", "Photography & videography", "Livestreaming & broadcast", "Content & creative direction"].map((s, i) => <div key={s}><span>0{i + 1}</span><b>{s}</b><i>↗</i></div>)}</div></section>
      <section id="about" className="soda-section about-section"><div className="soda-section-top"><span>07 / The studio</span><span>Creating beyond limits</span></div><div className="about-grid"><h2>Different<br /><em>by design.</em></h2><div><p>BlueHaven Studios is a multidisciplinary creative studio connecting brand strategy, design, technology, media and storytelling.</p><p>From school campaigns to business identities, websites and live productions, we turn ambitious ideas into experiences people remember.</p><a href="#contact">Start a conversation ↗</a></div></div></section>
      <section id="contact" className="soda-contact-section"><div className="soda-section-top"><span>08 / Let&apos;s make something</span><span>Available for projects</span></div><h2>Have a brave<br /><em>idea?</em></h2><div className="contact-links"><a href="mailto:bluehavens.studios@gmail.com">bluehavens.studios@gmail.com <span>↗</span></a><a href="tel:+2348068483718">+234 806 848 3718 <span>↗</span></a><a href="https://bluehavens.name.ng" target="_blank" rel="noreferrer">bluehavens.name.ng <span>↗</span></a></div><footer><span>BLUEHAVEN STUDIOS®</span><span>CREATIVITY BEYOND LIMITS</span><span>LAGOS / NIGERIA</span></footer></section>
    </main>
  );
};
